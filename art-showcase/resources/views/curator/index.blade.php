...
<div class="bg-white min-h-screen pb-20">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12">
        ...
        <!-- Filter Categories (BERSIH: Hanya Kategori Saja) -->
        <div class="flex flex-wrap justify-center items-center gap-3 mb-12">
            ...
            @foreach($categories as $category)
                @php
                    ...
                    // Logika Toggle: Jika aktif, link akan mereset filter. Jika tidak, link akan memilih kategori.
                    $url = $isActive 
                        ? route('artworks.index', ['search' => request('search')]) // Hapus param category
                        : route('artworks.index', array_merge(request()->all(), ['category' => $category->id]));
                @endphp

                <a href="{{ $url }}" 
                   class="px-6 py-2 rounded-full text-sm font-medium transition-all {{ $isActive ? 'bg-dark text-white shadow-lg' : 'bg-white text-gray-600 border border-gray-200 hover:border-dark hover:text-dark' }}">
                    {{ $category->name }}
                    @if($isActive) 
                        <!-- Tanda Silang Kecil jika aktif -->
                        <span class="ml-1 opacity-75">&times;</span> 
                    @endif
                </a>
            @endforeach
        </div>
